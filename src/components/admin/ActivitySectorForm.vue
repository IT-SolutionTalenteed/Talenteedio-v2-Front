<template>
    <div class="d-flex align-center justify-space-between mb-5">
      <div class="d-flex align-center ga-2">
        ...
        <span class="text-h6 font-weight-bold">{{ isEdit ? 'Modifier le secteur' : 'Nouveau secteur' }}</span>
      </div>
...
const save = async () => {
  ...
  try {
    if (isEdit.value) {
      ...
      showSnack("Secteur d'activité modifié avec succès")
    } else {
      await activitySectorService.create(form.value)
      showSnack("Secteur d'activité créé avec succès")
    }
