<template>
  <div class="container-xl">
    ...
    <div v-if="showForm || editingItem" class="card-body border-bottom">
      <h4 class="mb-3">{{ editingItem ? 'Modifier' : 'Créer' }} une compétence</h4>
      ...
      <table class="table table-vcenter card-table">
        ...
        <tbody>
          ...
          <tr v-else-if="items.length === 0">
            <td colspan="3" class="text-center text-muted">Aucune compétence de travail trouvé.</td>
          </tr>
...
const save = async () => {
  ...
  try {
    if (editingItem.value) {
      await skillService.update(editingItem.value.id, form.value)
      success.value = 'Compétence modifié avec succès'
    } else {
      await skillService.create(form.value)
      success.value = 'Compétence créé avec succès'
    }
    await load()
    cancelForm()
  } catch (err) {
...
const deleteItem = async (id) => {
  if (!confirm('Supprimer ce compétence ?')) return
  loading.value = true
  error.value = ''
  try {
    await skillService.delete(id)
    success.value = 'Compétence supprimé avec succès'
    await load()
  } catch (err) {
