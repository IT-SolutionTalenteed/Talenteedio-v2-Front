<template>
  <v-card rounded="xl" border elevation="0">
    <v-toolbar color="transparent" border="b" density="compact" class="px-2">
      ...
      <v-toolbar-title class="text-body-1 font-weight-semibold">{{ t('admin.referential.skills.title') }}</v-toolbar-title>
      <template #append>
        <v-btn color="primary" prepend-icon="mdi-plus" size="small" @click="router.push({ name: 'AdminSkillCreate' })">{{ t('admin.referential.add') }}</v-btn>
      </template>
    </v-toolbar>
    ...
    >
      <template #item.actions="{ item }">
        <v-btn icon="mdi-pencil" size="small" color="primary" variant="text" @click="router.push({ name: 'AdminSkillEdit', params: { id: item.id } })" />
...
const deleteItem = async (id) => {
  const ok = await confirmRef.value.open({ message: t('admin.referential.skills.confirmDelete') })
  if (!ok) return
  loading.value = true
  try {
