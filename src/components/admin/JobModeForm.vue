<template>
    <div class="d-flex align-center justify-space-between mb-5">
      <div class="d-flex align-center ga-2">
        ...
        <span class="text-h6 font-weight-bold">{{ isEdit ? 'Modifier le mode de travail' : 'Nouveau mode de travail' }}</span>
      </div>
...
const save = async () => {
  ...
  try {
    if (isEdit.value) {
      ...
      showSnack('Mode de travail modifié avec succès')
    } else {
      await jobModeService.create(form.value)
      showSnack('Mode de travail créé avec succès')
    }
