<template>
    <div class="d-flex align-center mb-5">
      <div class="d-flex align-center ga-2">
        ...
        <span class="text-h6 font-weight-bold">{{ isEdit ? t('admin.referential.jobModes.editTitle') : t('admin.referential.jobModes.newTitle') }}</span>
      </div>
    </div>
...
const goBack = () => router.push({ name: 'AdminJobModes' })
...
const save = async () => {
  ...
  try {
    if (isEdit.value) {
      await jobModeService.update(route.params.id, form.value)
      showSnack(t('admin.referential.jobModes.updated'))
    } else {
      await jobModeService.create(form.value)
      showSnack(t('admin.referential.jobModes.created'))
    }
