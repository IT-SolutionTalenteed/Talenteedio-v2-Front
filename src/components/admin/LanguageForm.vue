<template>
    <div class="d-flex align-center mb-5">
      <div class="d-flex align-center ga-2">
        ...
        <span class="text-h6 font-weight-bold">{{ isEdit ? t('admin.referential.languages.editTitle') : t('admin.referential.languages.newTitle') }}</span>
      </div>
    </div>
...
const save = async () => {
  ...
  try {
    if (isEdit.value) {
      ...
      showSnack(t('admin.referential.languages.updated'))
    } else {
      await languageService.create(form.value)
      showSnack(t('admin.referential.languages.created'))
    }
