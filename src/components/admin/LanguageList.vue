<template>
  <div class="container-xl">
    ...
    <div v-if="showForm || editingItem" class="card-body border-bottom">
      <h4 class="mb-3">{{ editingItem ? 'Modifier' : 'Créer' }} une langue</h4>
      ...
      <table class="table table-vcenter card-table">
        ...
        <tbody>
          ...
          <tr v-else-if="items.length === 0">
            <td colspan="3" class="text-center text-muted">Aucune langue de travail trouvé.</td>
          </tr>
...
const save = async () => {
  ...
  try {
    if (editingItem.value) {
      ...
      success.value = 'Langue modifié avec succès'
    } else {
      await languageService.create(form.value)
      success.value = 'Langue créé avec succès'
    }
    await load()
    cancelForm()
  } catch (err) {
...
const deleteItem = async (id) => {
  if (!confirm('Supprimer ce langue ?')) return
  loading.value = true
  error.value = ''
  try {
    await languageService.delete(id)
    success.value = 'Langue supprimé avec succès'
    await load()
  } catch (err) {
