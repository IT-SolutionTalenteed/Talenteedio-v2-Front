  ...
  <v-dialog v-model="dialog" max-width="500">
    <v-card rounded="xl">
      <v-toolbar color="primary" density="compact">
        <v-toolbar-title class="text-body-1">{{ editingItem ? 'Modifier' : 'Créer' }} une langue</v-toolbar-title>
        <template #append>
...
const save = async () => {
  ...
  try {
    if (editingItem.value) {
      ...
      showSnack('une langue modifié(e) avec succès')
    } else {
      await languageService.create(form.value)
      showSnack('une langue créé(e) avec succès')
    }
    await load()
    cancelForm()
  } catch (err) {
