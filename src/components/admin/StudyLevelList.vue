<template>
  <v-card rounded="xl" border elevation="0">
    <v-toolbar color="transparent" border="b" density="compact" class="px-2">
      ...
      <v-toolbar-title class="text-body-1 font-weight-semibold">{{ t('admin.referential.studyLevels.title') }}</v-toolbar-title>
      <template #append>
        <v-btn color="primary" prepend-icon="mdi-plus" size="small" @click="router.push({ name: 'AdminStudyLevelCreate' })">{{ t('admin.referential.add') }}</v-btn>
      </template>
    </v-toolbar>
...
const deleteItem = async (id) => {
  const ok = await confirmRef.value.open({ message: t('admin.referential.studyLevels.confirmDelete') })
  if (!ok) return
  loading.value = true
  try {
