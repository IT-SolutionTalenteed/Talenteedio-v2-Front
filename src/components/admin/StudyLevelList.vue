<template>
  <div class="container-xl">
    ...
    <div v-if="showForm || editingItem" class="card-body border-bottom">
      <h4 class="mb-3">{{ editingItem ? 'Modifier' : 'Créer' }} un niveau d\'étude</h4>
      ...
      <table class="table table-vcenter card-table">
        ...
        <tbody>
          ...
          <tr v-else-if="items.length === 0">
            <td colspan="3" class="text-center text-muted">Aucun niveau d\'étude trouvé.</td>
          </tr>
...
const load = async () => {
  ...
  error.value = ''
  try {
    const response = await studyLevelService.getAll()
...
const save = async () => {
  ...
  try {
    if (editingItem.value) {
      await studyLevelService.update(editingItem.value.id, form.value)
      success.value = 'Niveau modifié avec succès'
    } else {
      await studyLevelService.create(form.value)
      success.value = 'Niveau créé avec succès'
    }
    await load()
    cancelForm()
  } catch (err) {
...
const deleteItem = async (id) => {
  if (!confirm('Supprimer ce niveau d\'étude ?')) return
  loading.value = true
  error.value = ''
  try {
    await studyLevelService.delete(id)
    success.value = 'Niveau supprimé avec succès'
    await load()
  } catch (err) {
