<template>
  ...
  <v-dialog v-model="dialog" max-width="480">
    <v-card rounded="xl">
      <v-card-title class="pa-4 pb-2">{{ editingItem ? 'Modifier' : 'Créer' }} un niveau</v-card-title>
...
const save = async () => {
  ...
  try {
    if (editingItem.value) {
      ...
      showSnack('un niveau modifié(e) avec succès')
    } else {
      await studyLevelService.create(form.value)
      showSnack('un niveau créé(e) avec succès')
    }
    await load()
    cancelForm()
  } catch (err) {
