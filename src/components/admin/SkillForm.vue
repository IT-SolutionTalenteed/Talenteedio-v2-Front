<template>
    <div class="d-flex align-center mb-5">
      <div class="d-flex align-center ga-2">
        ...
        <span class="text-h6 font-weight-bold">{{ isEdit ? 'Modifier la compétence' : 'Nouvelle compétence' }}</span>
      </div>
    </div>
...
const save = async () => {
  ...
  try {
    if (isEdit.value) {
      ...
      showSnack('Compétence modifiée avec succès')
    } else {
      await skillService.create(form.value)
      showSnack('Compétence créée avec succès')
    }
