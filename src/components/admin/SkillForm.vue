<template>
    <div class="d-flex align-center mb-5">
      <div class="d-flex align-center ga-2">
        ...
        <span class="text-h6 font-weight-bold">{{ isEdit ? t('admin.referential.skills.editTitle') : t('admin.referential.skills.newTitle') }}</span>
      </div>
    </div>
...
const goBack = () => router.push({ name: 'AdminSkills' })
...
const save = async () => {
  ...
  try {
    if (isEdit.value) {
      await skillService.update(route.params.id, form.value)
      showSnack(t('admin.referential.skills.updated'))
    } else {
      await skillService.create(form.value)
      showSnack(t('admin.referential.skills.created'))
    }
