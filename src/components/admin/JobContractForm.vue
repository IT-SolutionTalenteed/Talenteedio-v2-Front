<template>
    <div class="d-flex align-center mb-5">
      <div class="d-flex align-center ga-2">
        ...
        <span class="text-h6 font-weight-bold">{{ isEdit ? t('admin.referential.jobContracts.editTitle') : t('admin.referential.jobContracts.newTitle') }}</span>
      </div>
    </div>
...
const goBack = () => router.push({ name: 'AdminJobContracts' })
...
const save = async () => {
  ...
  try {
    if (isEdit.value) {
      await jobContractService.update(route.params.id, form.value)
      showSnack(t('admin.referential.jobContracts.updated'))
    } else {
      await jobContractService.create(form.value)
      showSnack(t('admin.referential.jobContracts.created'))
    }
