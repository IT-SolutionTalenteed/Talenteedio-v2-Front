<template>
    <div class="d-flex align-center justify-space-between mb-5">
      <div class="d-flex align-center ga-2">
        ...
        <span class="text-h6 font-weight-bold">{{ isEdit ? 'Modifier le contrat' : 'Nouveau contrat' }}</span>
      </div>
...
const save = async () => {
  ...
  try {
    if (isEdit.value) {
      ...
      showSnack('Contrat modifié avec succès')
    } else {
      await jobContractService.create(form.value)
      showSnack('Contrat créé avec succès')
    }
